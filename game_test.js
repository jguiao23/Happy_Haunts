...
let chapterMirror = getChapterMirror();
  ...
  // Instantiate the TextScene with dialogue and optional positioning parameters
...
function draw() {
  ...
  }else if(gameState === "mininovel"){
    push();
    miniNovel.displayBox();
    pop();
    thirdNovel.displayText();
...
function mousePressed() {
  if (gameState === "novel") {
    ...
    miniNovel.nextLine();
    if(miniNovel.currentIndex=miniNovel.dialogue.length - 1){
      gameState = "novel4";
    }  
  }else if (gameState === "novel4") {
    ...
    if(finalNovel.currentIndex>=finalNovel.dialogue.length - 1){
      gameState = "novelMirror";
    }
  }else if (gameState === "novelMirror") {
    mirrorNovel.nextLine();
 }  
...
function getChapter3(){
  return[
    ...
    { name:"Ann",
      line: "At the moment, Bun Bun is on display in someone’s home. A young woman, she apparently purchased Bun off of something called the World Wide Web" 
    },
    { name:"Ann",
...
function getChapterMirror(){
  ...
  {
    name:"Ann",
    line:"Player Scratches Mirror off screen"
  },
  {
    name:"Ann",
